...
\date{13 July 2026 (living document; grows with each modeling session)}
...
learned model, and the ensemble and boosting methods that follow it. Each
equation is numbered and tied directly to the corresponding code in
\texttt{src/evaluate.py}, \texttt{notebooks/03\_baselines.ipynb}, and
\texttt{notebooks/04\_models\_ridge\_tree.ipynb}. New sections are added as
later sessions (XGBoost, error analysis) are built, so this remains a
...
\input{sections/01_evaluation_metrics}
\input{sections/02_baselines}
\input{sections/03_ridge_regularization}
\input{sections/04_ensembles_boosting}
