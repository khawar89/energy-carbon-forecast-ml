...
| Error analysis | Not started (Session 6 next; `results/test_predictions.csv` saved for it) |
...
**Session 5 CLOSED 14 Jul.** The single test evaluation has been run; results are FROZEN (no retuning on any split; see AGENTS.md "Post-test freeze"). Next exact action: **Session 6, error analysis.** Build `notebooks/06_error_analysis.ipynb` reading `results/test_predictions.csv` (762 per-row test predictions for all 8 models, saved by `src/train.py`): errors by country, by prediction year (2020 COVID break explicitly), and by emitter-size tier; re-verify on TEST data whether the Session 3 MAE-vs-percentage-error ranking reversal (Russia/Pakistan/Vietnam) survives before building any figure from it (see the 13 Jul decisions-log entry and `AGENTS.md` figure rules); figures to `results/figures/` with one modeling consequence each. Session 6 interprets, it does not re-fit. After that: Session 7 (reproducibility and publication pass), then the math/ error-analysis section and the LinkedIn post per `linkedin_drafts/Session5_PreRegistered_Drafts_WinsAndLoses.md` (Version B framing applies: positive point skill, interval includes zero).
