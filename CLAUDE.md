...
The journal-article work is private and its status is not tracked here. It lives
in `journal_article_private/`, which is excluded from this repository. Read that
folder's `AGENTS.md` and `CLAUDE.md` before any work on it.

Status detail previously kept in this file was moved on 23 August 2026 to
`journal_article_private/STATUS_HISTORY_from_public_CLAUDE.md`, because this file
is public and the manuscript is an unsubmitted, double-anonymized submission.
...
1. ~~Khawar reviews the published static site~~ DONE. Independently re-verified by Claude 15 Jul (browser-tested at desktop and mobile widths, dark and light: every number matches `results/model_comparison.csv`, `error_by_tier.csv`, and `error_by_country_persistence_top10.csv` exactly; the lens toggle re-ranks correctly; no horizontal overflow; og.png unfurl carries the caveat). No wrong information found. Full record: `linkedin_drafts/posted/2026-07-15_POSTED_session5-live-project.md`.
2. ~~Khawar reviews and publishes the LinkedIn post~~ DONE 15 Jul, using `linkedin_drafts/posted/2026-07-15_source-draft_live-project.md` (superseding the earlier `archive/2026-07-14_session5-7-final-text_SUPERSEDED.md` draft, written before the website existed). Exact posted text saved to `linkedin_drafts/posted/2026-07-15_POSTED_session5-live-project.md` per the standing convention.
3. ~~Add the verified live-page URL to the master CV~~ DONE 15 Jul. New "Independent Data Science Portfolio" section added to `3_CV_Latest_updated/00_Master_CV/CV_KhawarNaeem_V10_Master_Jul2026.docx` (V9 archived), plus "scikit-learn, XGBoost" added to the Technical & Methodological Skills Python line, using the "Approved CV wording" below almost verbatim and explicitly labeled self-directed/independent of QTTSC-affiliated research. Full change record in `3_CV_Latest_updated/CLAUDE.md`. PDF export still pending (no LibreOffice/poppler on this machine; Khawar to export manually).
...
- Test pre-registration and both LinkedIn drafts (`linkedin_drafts/archive/2026-07-14_session5-prereg-wins-and-loses.md`) written BEFORE the unlock; test pre-registration prepared with Claude's full guidance at Khawar's request and marked as such (validation pre-registration is his own).
...
- Final LinkedIn post text (Version B, real numbers, four prepared objections) written to `linkedin_drafts/archive/2026-07-14_session5-7-final-text_SUPERSEDED.md` for Khawar to publish himself.
...
- Khawar published the LinkedIn post using `linkedin_drafts/posted/2026-07-15_source-draft_live-project.md`
  with the native social-card image; exact posted text saved to
  `linkedin_drafts/posted/2026-07-15_POSTED_session5-live-project.md` per the standing convention.
